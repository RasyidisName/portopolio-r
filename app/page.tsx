      ...
      <section className="deck" aria-live="polite">
        <AnimatePresence initial={false} mode="wait">
          <motion.div key={active} className="slide" initial={{ opacity: 0, x: 70 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -70 }} transition={{ duration: .38, ease: [0.22, 1, 0.36, 1] }}>
            ...
            {active >= 2 && active < total - 1 && <ProjectSlide project={projects[active - 2]} index={active - 2} editMode={editMode} onImage={() => pickImage(active - 2)} />}
...
function ProjectSlide({ project, index, editMode, onImage }: { project: typeof initialProjects[number]; index: number; editMode: boolean; onImage: () => void }) {
  return <div className="project-slide"><div className="project-image-column"><button className={`project-image ${editMode ? 'editable-image' : ''}`} onClick={onImage} aria-label="Replace project image"><img src={project.image} alt={project.title} /></button>{editMode && <span className="image-hint"><Upload size={13} /> Click to replace</span>}<span className="portrait-caption">{project.number} / 0{initialProjects.length} — Selected work</span></div><div className="project-copy"><div className="eyebrow"><span className="eyebrow-line" /> <Editable editMode={editMode}>{project.category}</Editable></div><h1><Editable editMode={editMode}>{project.title}</Editable></h1><p className="lead"><Editable editMode={editMode}>{project.description}</Editable></p><div className="tag-list">{project.tags.map((tag) => <span key={tag}><Editable editMode={editMode}>{tag}</Editable></span>)}</div><a href="#contact" className="text-link">View case study <ArrowUpRight size={15} /></a></div></div>
}
